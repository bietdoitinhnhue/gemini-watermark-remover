...
test('createLocalStaticPreviewServer should serve local preview assets over loopback HTTP', async () => {
  ...
  try {
    await writeFile(path.join(tempDir, 'video-remover.html'), '<h1>preview</h1>');
    ...
    try {
      ...
      const pageResponse = await fetch(new URL('video-remover.html', server.url));
...
test('withLocalStaticPreviewPage should convert file pages to temporary HTTP URLs and close the server', async () => {
  ...
  try {
    const pagePath = path.join(tempDir, 'video-remover.html');
    ...
    assert.match(callbackUrl, /^http:\/\/127\.0\.0\.1:\d+\/video-remover\.html$/);
